...
void create_table(sqlite3 *db, const char *table_name)
{
  sqlite3_stmt *stmt;
  char query[110];
  sprintf(query, "%s%s%s", "CREATE TABLE `", table_name, "` (command TEXT, output TEXT, start_time TEXT, finish_time TEXT, indicator INTEGER);");
...
void create_row(sqlite3 *db, const char *table_name)
{
  sqlite3_stmt *stmt;
  char query[100];
  sprintf(query, "%s%s%s", "INSERT INTO `", table_name, "` VALUES (NULL, '', NULL, NULL, NULL);");
  if (sqlite3_prepare_v2(db, query, -1, &stmt, NULL) != SQLITE_OK) {
...
void add_command(sqlite3 *db, const char *table_name, const void *command, size_t size)
{
  sqlite3_stmt *stmt;
  char query[100];
  sprintf(query, "%s%s%s%s%s", "UPDATE `", table_name, "` SET command=? WHERE rowid = (SELECT MAX(rowid) FROM `", table_name, "`);");
  if (sqlite3_prepare_v2(db, query, -1, &stmt, NULL) != SQLITE_OK) {
...
void add_output(sqlite3 *db, const char *table_name, const void *data)
{
  sqlite3_stmt *stmt;
  char query[100];
  sprintf(query, "%s%s%s%s%s", "UPDATE `", table_name, "` SET output=output || ? WHERE rowid = (SELECT MAX(rowid) FROM `", table_name, "`);");
  if (sqlite3_prepare_v2(db, query, -1, &stmt, NULL) != SQLITE_OK) {
...
void add_start_time(sqlite3 *db, const char *table_name)
{
  ...
  sprintf(query, "%s%s%s%s%s", "UPDATE `", table_name, "` SET start_time=? WHERE start_time is NULL AND rowid = (SELECT MAX(rowid) FROM `", table_name, "`);");
  if (sqlite3_prepare_v2(db, query, -1, &stmt, NULL) != SQLITE_OK) {
...
void add_finish_time(sqlite3 *db, const char *table_name)
{
  ...
  sqlite3_stmt *stmt;
  char query[100];
  sprintf(query, "%s%s%s%s%s", "UPDATE `", table_name, "` SET finish_time=? WHERE rowid = (SELECT MAX(rowid) FROM `", table_name, "`);");
  if (sqlite3_prepare_v2(db, query, -1, &stmt, NULL) != SQLITE_OK) {
...
void add_indicator(sqlite3 *db, const char *table_name)
{
  ...
  sqlite3_stmt *stmt;
  char query[100];
  sprintf(query, "%s%s%s%s%s", "UPDATE `", table_name, "` SET indicator=? WHERE rowid = (SELECT MAX(rowid) FROM `", table_name, "`);");
  if (sqlite3_prepare_v2(db, query, -1, &stmt, NULL) != SQLITE_OK) {
